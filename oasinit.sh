...
#nNginx, mysql, PHP download and package name variable
packdownurl="http://package.brotlab.net:8086/package/"
initurl="http://package.brotlab.net:8086/package/init/"
...
function mysql_install() {
    ...
    chown -R mysql:mysql /data/mysqllogs /usr/local/mysql/data
    chmod 777 /data/logs
    #rpm -qa |grep wget &>/dev/null|| yum install wget -y &>/dev/null
    #wget $packdownurl$mysqlpackname $initurl$mysqlinitname $initurl$mysqlconfname >/dev/null
    wget $initurl$mysqlinitname $initurl$mysqlconfname >/dev/null
...
function zabbix_agent_install() {
    if `grep "CentOS release 5.* (Final)" /etc/issue >/dev/null`;then
        rpm -qa | grep zabbix-2.2.5-1 || rpm -Uvh http://package.brotlab.net:8086/zabbix/el5/zabbix-2.2.5-1.el5.x86_64.rpm
        rpm -qa | grep zabbix-agent-2.2.5-1 || rpm -Uvh http://package.brotlab.net:8086/zabbix/el5/zabbix-agent-2.2.5-1.el5.x86_64.rpm
    else
        rpm -qa | grep zabbix-2.2.5-1 || rpm -Uvh http://package.brotlab.net:8086/zabbix/el6/zabbix-2.2.5-1.el6.x86_64.rpm
        rpm -qa | grep zabbix-agent-2.2.5-1 || rpm -Uvh http://package.brotlab.net:8086/zabbix/el6/zabbix-agent-2.2.5-1.el6.x86_64.rpm
    fi
        ...
        wget -q -P /etc/zabbix/scripts/ http://package.brotlab.net:8086/zabbix/config/monitor_mysql
        ...
        wget -q -P /etc/zabbix/zabbix_agentd.d/ http://package.brotlab.net:8086/zabbix/config/mysql_status.conf
